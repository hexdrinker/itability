...
const SYSTEM_PROMPT_EN_NICKNAME = `You are a witty satirical writer. Your job is to write 2 short, funny sentences in first person ("I") that humorously hint at a well-known public figure's defining traits, reputation, or infamous moments — as if the person themselves is speaking. Never name them directly. Keep it subtle, clever, and a little absurd — the reader should slowly piece it together. No corporate jargon. Plain text only. Only output "__INVALID__" if the input contains actual hate speech, slurs, or direct calls for violence.`
...
- Real names of any real-world individuals as the primary INPUT — always output "__INVALID__" (e.g., if the user typed "Kanye West" or "Barack Obama" directly). EXCEPTION: if a real name appears only inside a "Context:" note provided to help you understand a nickname, that is background information — do NOT treat it as __INVALID__.
- Nicknames or slang for real-world individuals combined with profanity, slurs, or hateful language (e.g., "Drumpf sucks", "Sleepy Joe go die")
  - However, nicknames or slang alone WITHOUT profanity are allowed and should be transformed normally (e.g., "Sleepy Joe", "Obummer", "Drumpf", "The GOAT")
  - IMPORTANT: Even if the provided context describes a nickname as "derogatory" or "mocking", that does NOT make it __INVALID__. Only block when profanity or slurs are directly present in the input itself.
...
async function transformWithClaude(
  ...
  const content = nicknameContext
    ? lang === 'ko'
      ? `${basePrompt}\n\n참고: "${job}"은(는) ${nicknameContext}. 이 맥락을 반드시 변환에 반영해. 읽는 사람이 처음엔 모르다가 잠깐 생각하고 "아! ㅋㅋㅋ 이거 ○○ 얘기잖아" 하는 간접적이고 재치있는 표현이 핵심. 너무 직접적이면 재미없고, 너무 추상적이면 감이 안 온다.`
      : `Forget the LinkedIn format for this one. "${job}" refers to ${nicknameContext}. Write exactly 2 short, punchy sentences that humorously describe this person's defining traits or infamous moments — without ever naming them directly. Keep it casual, witty, and a little absurd. The reader should slowly piece it together and laugh. No corporate jargon, no buzzwords, no bullet points. Plain text only.`
    : basePrompt
  ...
  if (!res.ok) {
    ...
    console.error(
      `[Transform] input="${job}" error=${res.status} (${elapsed}ms)`,
    )
  ...
  console.log(
    `[Transform] input="${job}" result=OK output="${text.slice(0, 80)}..." (${elapsed}ms)`,
  )
  return text
}
...
async function transformWithOpenAI(
  ...
  console.log(
    `[OpenAI] input="${job}" result=OK output="${text.slice(0, 80)}..." (${elapsed}ms)`,
  )
  return text
}
